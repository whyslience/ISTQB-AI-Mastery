...
/** Stable per-page slug; appends -2, -3… when the same title repeats (e.g. LO block + body). */
function makeUniqueSlugFactory() {
...
export default async function ChapterReview({ params }: { params: Promise<{ chapterId: string }> }) {
  ...
  // Extract headings: full list drives rendered ids; TOC omits ### duplicates under Learning Objectives (before ---).
  const nextUniqueSlug = makeUniqueSlugFactory();
  const headingRegex = /^(#{1,4})\s+(.+)$/gm;
  const loRange = findLearningObjectivesSectionRange(markdownContent);
  const allHeadingMeta: {
    id: string;
    text: string;
    level: number;
  ...
  while ((match = headingRegex.exec(markdownContent)) !== null) {
    ...
    const line = lineNumberAtIndex(markdownContent, match.index);
    const id = nextUniqueSlug(text);
    const skipToc = shouldSkipHeadingInToc(level, line, loRange);
    allHeadingMeta.push({
      id,
      text,
      level,
      includeInToc: !skipToc,
    });
  }

  const mdHeadingIds = allHeadingMeta.map((h) => h.id);
  const headings = allHeadingMeta
    .filter((h) => h.includeInToc)
    .map(({ id, text, level }) => ({ id, text, level }));

  const mdHeadingCount = allHeadingMeta.length;
  let quizHeadingId: string | null = null;

  // Add Practice Questions to TOC if it exists
  if (chapter.quiz && chapter.quiz.length > 0) {
    const quizTitle = `${mdHeadingCount + 1}. Practice Questions / Câu hỏi Luyện tập`;
    quizHeadingId = nextUniqueSlug(quizTitle);
    headings.push({
      id: quizHeadingId,
      text: quizTitle,
      level: 2,
      ...
      <div className="w-full max-w-4xl min-w-0">
        ...
        ">
          <ReactMarkdown 
            ...
            components={{
              h1: ({ children }) => (
                <h1 id={nextMarkdownHeadingId(children)}>{children}</h1>
              ),
              h2: ({ children }) => (
                <h2 id={nextMarkdownHeadingId(children)}>{children}</h2>
              ),
              h3: ({ children }) => (
                <h3 id={nextMarkdownHeadingId(children)}>{children}</h3>
              ),
              h4: ({ children }) => (
                <h4 id={nextMarkdownHeadingId(children)}>{children}</h4>
              ),
              p: ({ children }) => {
                const extractText = (node: unknown): string => {
                  ...
                  if (typeof node === "object" && node !== null && "props" in node) {
                    ...
                    if (p?.children != null) return extractText(p.children);
                  }
                  return "";
                };
                const plain = extractText(children);
                const pair = splitEnViPair(plain);
                if (pair) {
                  return (
                    <>
                      <p className="en-text">{pair.en}</p>
                      <p className="vi-text">{pair.vi}</p>
                    </>
                  );
                }
                const isVi = hasVietnameseChars(plain);
                return <p className={isVi ? "vi-text" : "en-text"}>{children}</p>;
              },
              li: ({ children }) => {
                const extractText = (node: unknown): string => {
                  if (node == null) return "";
                ...
                if (pair) {
                  return (
                    <li className="bilingual-li-split">
                      <div className="en-text">{pair.en}</div>
                      <div className="vi-text">{pair.vi}</div>
                    </li>
                  );
                }
                const isVi = hasVietnameseChars(plain);
                return (
                  <li className={isVi ? "vi-item" : "en-item"}>
                    {children}
                  </li>
                );
              },
